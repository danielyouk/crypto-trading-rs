...
- **답변은 한국어 존댓말로** 합니다(`-습니다`, `-세요`). 코드·파일 경로·명령어는 그대로 둡니다.
- **파이썬은 `.venv/bin/python`으로 실행**합니다. 확인용 임시 스크립트는 결과를 얻은 즉시 지우고, 새 패키지는 `requirements.txt`에 버전을 고정해 추가한 뒤 설치합니다.
- **결과를 먼저 보고 교재를 씁니다.** 실습 프롬프트를 실제로 돌려 본 뒤 무엇을 가르칠지 정합니다. 교재에는 실습 조건과 읽는 법만, 그 실행의 숫자는 대본의 촬영 참고값에만 둡니다(집필 원칙 §12).
- **작업 단위가 끝나면 커밋하고 푸시**합니다. 재빌드와 일관성 검사를 통과한 뒤, 관련 파일만 골라 담고 무엇을 왜 바꿨는지 메시지에 적습니다. 논의가 끝나지 않았으면 커밋하지 않습니다.
- **강의 아이디어는 먼저 제안만** 합니다. `docs/lecture-ideas.md`에 넣을 문구를 보여 주고, 승인받은 뒤에 추가합니다.
