...
> 이 빌드는 샌드박스 안에서 돌리면 `/mnt/c` 파일 스캔 때문에 몇 분씩 멈춘 것처럼 보일 수 있습니다. 샌드박스 밖에서 실행하면 2초 안에 끝납니다.

---

## 커밋 전 일관성 검사

교재나 대본을 고쳤으면 커밋 전에 한 번 돌립니다. 프롬프트 4칸 골격, 구형 표기, 차트 형식, HTML 재빌드 누락, 대본↔교재 파일명 불일치를 잡습니다.

```bash
.venv/bin/python runners/check_lecture_consistency.py
```

FAIL이 있으면 종료 코드 1입니다. 검사 항목과 예외 처리 방법은 [`docs/course/집필_원칙.md`](docs/course/집필_원칙.md) 10~11절에 있습니다.
