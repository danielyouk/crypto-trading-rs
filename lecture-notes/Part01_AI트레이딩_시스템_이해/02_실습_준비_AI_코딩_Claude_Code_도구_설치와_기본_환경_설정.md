...
```text
[1단계: 사람이 직접 수행] 
Node.js 설치 ──> 터미널에서 Claude Code 설치 (`npm install -g @anthropic-ai/claude-code`) ──> 로그인

[2단계: AI에게 위임] 
작업 폴더에서 `claude` 실행 ──> 프롬프트로 Python 가상환경 및 라이브러리 설치 요청

[3단계: 사람이 눈으로 확인] 
삼성전자 어제 종가 및 데이터프레임이 정상 출력되는지 화면 대조
```

---

## 실습 — 실습 01: Claude Code 설치 및 환경 구축

### Step 1. 작업 폴더 생성 및 Claude Code 시작

컴퓨터의 원하는 위치(예: 바탕화면 또는 작업 드라이브)에 `ai-trading` 폴더를 만들고, 터미널(Windows의 경우 PowerShell 또는 WSL/Git Bash, Mac의 경우 Terminal)에서 해당 폴더로 이동한 뒤 Claude Code를 실행합니다.
