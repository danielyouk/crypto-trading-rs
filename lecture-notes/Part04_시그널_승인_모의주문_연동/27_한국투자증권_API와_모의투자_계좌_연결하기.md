...
이 클립이 끝나면, 여러분은 **한국투자증권(KIS) Developers 포털에서 모의투자 API 키를 발급**받고, 보안 환경설정 파일(`.env`)을 통해 인증 정보를 안전하게 격리한 뒤, **파이썬 스크립트로 OAuth2 접근 토큰을 발급받아 모의투자 계좌의 예수금 및 보유 주식 잔고를 터미널에 깔끔한 표로 출력**하는 데 성공하게 됩니다.
...
4. 발급된 토큰으로 '주식 잔고조회(TTTC8434R)' API를 호출하여 내 계좌의 총 예수금, 총 평가금액, 총 손익률을 터미널에 요약 표로 출력해줘.
5. 전체 코드는 'kis_auth.py'로 저장해줘.

실행 완료 후 터미널에 내 모의투자 계좌의 예수금 요약표를 출력해서 정상 연결을 증명해줘. (Verification)
```
...
- [ ] 터미널에 내 모의투자 계좌의 총 예수금(예: 500,000,000원)과 평가 잔고 표가 성공적으로 출력되었다.
