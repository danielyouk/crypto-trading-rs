...
from typing import Iterable, Literal, Tuple, Dict, Any, Optional, cast
...
def _prepare_prices(price_a: pd.Series, price_b: pd.Series) -> Tuple[pd.Series, pd.Series, pd.Index]:
    ...
    a = cast(pd.Series, price_a.copy())
    b = cast(pd.Series, price_b.copy())
    a.index, b.index = pd.to_datetime(a.index), pd.to_datetime(b.index)
    a, b = cast(pd.Series, a.sort_index()), cast(pd.Series, b.sort_index())
    
    if a.index.duplicated().any(): 
        a = cast(pd.Series, a[~a.index.duplicated(keep="last")])
    if b.index.duplicated().any(): 
        b = cast(pd.Series, b[~b.index.duplicated(keep="last")])
        
    common = cast(pd.DatetimeIndex, a.index).intersection(b.index).sort_values()
    if len(common) == 0:
        raise ValueError("No overlapping index between price_a and price_b")
        
    a, b = cast(pd.Series, a.reindex(common)), cast(pd.Series, b.reindex(common))
...
def run_pairs_backtest_vectorized(inp: PairsBacktestInput) -> PairsBacktestOutput:
    ...
    _min_p = cast(int, inp.min_periods) if inp.min_periods is not None else inp.window
    rm = cast(pd.Series, spread.rolling(window=inp.window, min_periods=_min_p).mean())
    rs = cast(pd.Series, spread.rolling(window=inp.window, min_periods=_min_p).std(ddof=0))
    ...
    position_for_pnl = cast(pd.Series, position.shift(1))
