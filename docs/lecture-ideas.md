...
- *"But aren't jumps and crashes a fundamental nature of stocks? If we just filter them out, aren't we ignoring reality? Is there a way to fix the math instead of throwing away the stock?"* → Yes! This leads to advanced quant techniques. Instead of a simple Simple Moving Average (SMA), we can use an Exponential Moving Average (EMA) which "forgets" old jumps faster. Better yet, we can use a **Kalman Filter** or **Jump-Diffusion Models** which mathematically detect a "regime change" (a permanent jump) and instantly reset the moving average to the new level, completely eliminating the 60-day "ghost signal" period. For this course, we use the 10% exclusion filter for simplicity, but fixing the math is the true institutional approach.
